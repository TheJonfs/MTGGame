...
| M3 | Vocabulary expansion 3 | Control Magic (control change), `destroy` + removal suite (Doom Blade, Terror, Swords, Wrath, Pyroclasm), reanimation, regrowth, discard (both modes), legend rule SBA, trigger conditions beyond `self` (ADR-021) + repeatable draw triggers, parameterized scopes (ADR-020) + first tribal card, Drana, Mystic Snake. Pool reaches ~100 cards. | Full ceiling implemented except reserved words. |
...
Status: M2a complete (S2, 2026-08-19). Session 3 brief cut (M2b). M3.5 replay viewer added between M3 and M4 per Chris.
