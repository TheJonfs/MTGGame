...
- **Playtest feedback round (Chris's first game, E vs journeyman-D seed 42):** (1) main phases + end step are now built-in stops on the player's own turn (pause once per step, then flow); (2) **fixed: unplayable duplicate lands** — R-029 dedups hand actions by cardId, so only one Island carried the playLand action; the controller now aliases every hand copy to the enumerated action (all copies glow, any copy clicks); (3) castable highlight is now bright gold with a glow, offset from every frame color; (4) inspector defaults to the printed card; (5) invisible-attack fix — the play screen re-renders on step changes and narrates un-pausable combat ("Opponent attacks with X", "You take N combat damage") since ADR-014 auto-takes pass-only windows; (6) **fixed: checkerboard on land tiles** — halftone-rosette moiré from the browser's single-step downscale of the Beta scans; `scripts/art-tiles.py` (wired into `pnpm art:fetch`) pre-generates prefiltered 160px tile derivatives and tiles fall back to full art if absent; (7) wood table background is one viewport-locked cover image instead of a visibly-seamed 900px tile.
...
2. ~~R-029's hand dedup shows in the UI~~ — **resolved in the playtest round** (Chris hit it as "can't play some Islands"): the controller aliases every hand copy of a castable/playable cardId to the enumerated action.
