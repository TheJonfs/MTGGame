...
export function loadCatalog(dir: string): Catalog {
  ...
  return catalogFrom({ regions: read("regions.json"), towns: read("towns.json"), opponents: read("opponents.json"), starters: read("starters.json"), dungeons: read("dungeons.json"), quests: read("quests.json") });
}
