...
**The world path and the broader seat audit (Chris: the report came from world play; is anything tied to a hardcoded seat?).** In the world the human is always seat 0 (`prepareDuel` seats the player first; the coin flip only picks who acts first), so the label defect could not have produced it there. Audited for hardcoded seats: the **engine** has none in rules or effects — every trigger collector is controller-relative (the landfall collector compares the entering land's controller with the Crab's, so an opposing Crab never fires on your land; the mill effect reads the chosen target; `players("you"/"opponent")` are the resolving item's controller and its opponent), and the only literal 0/1 indices are symmetric array reads (life, libraries, graveyards for the view and the result). The **cards/agents** carry none (combat-sim's two-seat life array). The **UI's** literal seats are the world's by-construction seating (the human at players[0]), the replay viewer's player-0-as-You convention (LogPanel, Rail), and the fixed `actionLabel`. An engine-level regression now pins the Crab from BOTH seats with a Crab on each side (`r8-crab-seats.test.ts`): one trigger, chosen at the opponent, mills the opponent's library by three and nothing else; the self-aim mills only the chooser. **What the world report most plausibly was**: a duel where the opponent also ran Crabs (Tessaly, Pell, Varro, Quill carry three or four) — the opponent's land fires the opponent's Crab, its AI aims at the player, and the play-by-play shows "Opp: Trigger targets → You" beside the player's own "You: Trigger targets → Opponent" a moment earlier; both mills happen and the player's cards go while the log also reports the opponent's. Without the seed that stays a reading; the last six duels of a save keep their replay log (the rail's "Recent duels"), so if the game is still in that save its log settles it.

## Card change (Chris, 2026-09-15): The Unwinder's ping is 3 damage

**The change.** The Unwinder's RETURNED_TO_HAND trigger deals **3 damage to any target** (was 1) — the def's effect, its text, the pool-registry row, `cards.md` (regenerated) and the as-printed render v2 (Chris's delivery, verified word-for-word: only the number changed; installed at 745px, the MANIFEST row notes the re-drop). No vocabulary touched. Two S22 fixtures re-based: the engine's own bounce cost now kills the 2/2 Bears it pinged (was 1 damage on them), and the opponent's Boomerang costs them 3 (20 → 17, was 19). Default tier 602 passed / 2 skipped; typecheck clean.

**Measured (`pnpm lord-sim --games 20`, the Unwinder at Spiral Spire under The Risen Tide — the lord's kill rate over the stock references; life variants hunted 15 / base 30 / grown 50 at empowerment steps 0 / 75 / 100):** hunted 15: 61% · 88% · 95%; base 30: 87% · 82% · 98%; grown 50: 78% · 91% · 98%. The Unwinder was already the sharpest lord at his base; three points a bounce makes every Boomerang, Man-o'-War and Channeler a Lightning Bolt beside the tide — worth a look in world play against a green road (the Bears-class bodies now die to a single bounce), and a Lab cell if the number wants a second opinion (the lords are in the pickers). The pre-change lord-sim rows are not on record in this format; the S22b handoff's tables were the last run.
