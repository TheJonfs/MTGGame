...
## Post-session (Chris's playtest report, 2026-09-21): menace and the offer to block

Reported: attacked by two creatures (one with menace) holding two untapped and two tapped creatures, no block offered. **Not reproducible**: the engine's `blockerChoices` and the controller's `enterBlockers` were driven through that exact board (a real game: the human's turn-one attack taps two Pikers, the AI swings a Brute and a Baloth) and offer every legal block — `packages/ui/src/play/menace-offer.test.ts` pins it. What could have hidden the truth: the attack narration named PRINTED keywords only (a granted menace/flying — the Obsidian Observatory's activation — never showed), and a block step the engine skips under ADR-014 (a lone "done" is never asked) said nothing. Fixed: the narration and the combat pause now read the LIVE keywords and, when no block will be offered while you have creatures standing, say why per attacker in the engine's terms. If it happens again, the log is the instrument: Recent duels → replay → Download log → `pnpm blocks-audit <file>` (Concern 8: the report may be a granted-keyword case — Isaura's ground — or a UI moment the tests do not reach; the audit will say which).
