...
| `townsPer100Cells` | `{"civilized":0.25,"approach":0.12,"wild":0}` | towns per 100 passable cells, by region tier | Town density per region (S16 uniform towns): count = max(floor for civilized/approach = 1, round(density × area/100)). Every non-wild region has ≥1 town, so every colour has a home town. |
