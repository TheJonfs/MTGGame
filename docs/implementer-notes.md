...
- `vercel.json` (repo root; a twin at `packages/ui/vercel.json` covers a project whose Root Directory was auto-detected as the UI package — same build script via `../../`, output `dist`): install `pnpm install --frozen-lockfile`, build `pnpm build:web`, output `packages/ui/dist`, SPA rewrites for the pathname routes (`/world`, `/play`, `/gallery`, `/viewer`, `/sound`) excluding `/assets/`, `/real-art/`, `/audio/`, and the `/__*` dev endpoints (they 404 cleanly; the gallery's registry fetch catches).
- **Both mounts are in the repo** (S27 r3, Chris): `data/art/real` (Scryfall art) and `assets/audio` (the FLAC library) are committed and COPIED at build into `public/real-art` and `public/audio` (both ignored) — no network at build, the deploy shows and sounds like dev. `ART_FETCH=1` opts into a Scryfall refresh.
...
## Deploy playtest (post-S27, live on Vercel)

- **`pnpm blocks-audit <duel.json> [--seat 0|1]`** — replays a saved duel log to every DECLARE BLOCKERS request the seat was asked and prints the attackers (with keywords), the seat's creatures, every block the engine OFFERED, and the action taken. It is `replayToDecision` at each blocker index, nothing re-implemented — the first tool to reach for when a player reports "I couldn't block". The seat defaults to the human's (`spec.players[i].agent === "human"`).
- **Duel logs are downloadable now**: the viewer's transport bar has "Download log" (`duel-<seed>-<turns>t.json`), and "⟵ Back" when the viewer was launched from a game. The journey stays MOUNTED (hidden) under the replay — Back returns to the exact screen that launched it (the result panel included); `WorldApp`'s `paused` prop holds the screen-driven music effect still meanwhile. **The wrapper must be `display: contents`** — a plain div breaks the `#root → .app { height: 100% }` chain and every `.app` screen grows with its content (the duel's prompt bar slid off the screen with the play-by-play log; Chris caught it the same day). The world's "Recent duels" rail panel keeps the last six duels' logs, so a report can be filed after the fact.
- **Menace beside a plain attacker** (Chris's report): engine test 9d pins that with one blocker the plain attacker's block is offered and the menace one withheld; the controller builds the blockers phase from the engine's offer without further filtering. If the report recurs, get the log (Recent duels → replay → Download log) and run blocks-audit on it.
- **The tavern's pour is budgeted by ring** (`rumorsPerTown` 1/2/3), actionable first: a live trail step (rotating among live chains on the rumor epoch; a trail to an opened door is moot), the manalink pointer, then lore. A civilized board with live trails therefore never pours lore — a deliberate consequence flagged in the handoff.
- **The cast prediction pays a creature's own ETB life cost now** (view-sim's `castSpell` branch: mandatory ENTERS_BATTLEFIELD `loseLife you` / `damage to you`), so the evaluator's lethal floor prices a suicide cast. It still predicts nothing else an ETB does — a future "ETB that hurts us" (sacrifice, discard) needs its own line here, not a new gate.
- **The tapper wants a creature**: `tapperGated` holds outright when the opponent has no untapped creature, and view-sim's `tapTarget` prices by what is tapped (creature by power; land ≈ nothing). Mana denial by tapping lands is deliberately not a line.
- **Look mode overscrolls by a third of the view** (`viewportOrigin`, pan ≠ 0 only). The map frame is drawn at the MAP's bounds, not the window's, and the strip beyond wears the `rough-faint` hatch (interiors: `chisel`). Anything that maps clicks to cells is unaffected — the cell rects stay inside the map; beyond is inert paper.
- **`docs/reference/` is generated and sync-tested** (`pnpm reference`): edit the data or the deck tables, never the markdown. The renderer (`packages/world/src/reference-docs.ts`) is the place to add a column; world already depends on sim's deck tables through `enemyDeck`.
- **Custom-art borders live in the canonical.** Five Gemini renders carried a ~25–40px light border inside the 1:1 image; a 5:4 centre crop keeps the sides. The re-crop trims contiguous columns/rows brighter than 232 (+8px) before the window — the scan in the deploy-playtest session is a ten-line Pillow script; run it over `packages/ui/public/custom-art` after any new render lands.

## S28 lessons (the Heart's roots, the legacy's flags, five one-drops)

- **The EVENT stream is a viewer's stream, not a ledger.** `MatchResult.log` carries CARD_DRAWN / SHUFFLED / SPELL_CAST / DIES / DAMAGE / ATTACHED / MILLED / TRIGGER_NO_TARGETS — no ZONE_CHANGE. A sim that wants "what stood at the end" reads `finalStateSerialized`; "did X leave the battlefield" reads DIES (exile and bounce are invisible — say so in the table).
- **Turn numbers are the game's, not the seat's.** With `startingPlayer` 0 the heart's first turn is turn 2; "turn-one flower" means the heart's FIRST OWN turn. The S28 heart-sim reads `turn ≤ 2` for the cast; the first diagnosis of a "withheld cast" was a pass on the OPPONENT's turn — check `activePlayer` before blaming the enumerator.
- **The master priced a body over an engine.** The cast prediction had no value for a `createLaw` trigger, so Faerie Formation (16.7) outscored the Manafleur (15.9) with five roots up and the flower bloomed a turn late in ~15% of games. A permanent that grows laws now carries +2.5 in the prediction (data-driven off the vocabulary word, not the card). Pinned in the roots test.
- **Trigger, not lifelink.** Spirit Link's gain is a stack trigger: at 2 life the aura's controller dies to the enchanted creature's 2 damage before the gain resolves (fixture C). The DEALS_DAMAGE collector is the S23 shape with an any-recipient DAMAGE listener; `player` in the context is set only when a player was damaged.
...
## S31 lessons (the Traumatizer's anchor, the Artisan, the five roads)

- **Controller-wide observers are a filter on the existing collector, not a new collector.** "Whenever a creature you control deals damage to a player" is the S23 damage collector with two more condition reads (`controller` relative to the observer, `type` of the source). Nothing Traumatizer-shaped lives in the engine; the same three-field condition serves any future "creatures you control have/do X" observer. Resist the urge to key a collector on a card.
- **`who: "eventPlayer"` rides the same payload as `eventDamage`** and is confined the same way (damage triggers only). The `Who` union is shared by many effects, so a new member has to be handled everywhere `players(who)` is switched — the init context throws, the predictors read it as the opponent.
- **A "when you cast" trigger's source is the STACK object.** The SPELL_CAST event now carries the stack object's id; the cast card's own `zone: "stack"` triggers pend with it as source, so they go on the stack above the spell and survive a counter. Don't scan the battlefield for it — the card is not there yet.
- **An edict is an N-times request loop plus a batch move.** One `chooseSacrifice` per pick (a lone candidate forced, ADR-014), then `moveBatchToGraveyard` so the permanents leave together (701.17). The request carries the resolving ability as its source so the chooser can tell an annihilator from an Altar.
- **Insertion targets in `effect-context.ts`: the shared `searchOp`/`sharedOps` factories are spread into BOTH contexts.** An edit that anchors on `async searchLibrary(` lands inside `searchOp` and the init context inherits it (the "specified more than once" error). Anchor new game-context methods on the game context's own spread block.
- **`sim` never imports `world`.** A fuzz test that wants the starters reads `data/world/starters.json` directly; `@shandalar/world` imports `sim`, not the reverse.
- **Factor a pricing curve the first time two callers need it.** The S29 mill curve lived inline in view-sim's mill case; the attack planner needed the same numbers for the Traumatizer, so it became `millValue` in the evaluator — the two predictions cannot drift now.
- **The R-table is the tier-3 formula with no override** (Scrubland 10, Demonic Tutor 30, Mystic Snake 50, the Artisan 100): "the implementer aligns" meant "don't override".

## S32–S33 lessons (the Escort, the yardstick, the matrix)

- **A gate that names a step must use the engine's step names.** `Step` is the source (`packages/engine/src/state.ts`: "END", not "END_STEP"). S28's cantrip window compared against a name that never occurs, and nothing noticed for four sessions because the test pins used the same wrong string. When a pin and its gate share a literal, the pin proves nothing — read the literal off the engine's type in the test, or at least grep the engine for it once.
- **"In response" needs an OPPONENT's item on the stack.** `view.stack.length > 0` is true when our own spell is on the stack (right after we cast it, with priority back), so a "hold it for the opponent's end step" rule leaks every own-turn cast that follows one of ours (S33 Part 4 measured 56–92% of own-turn Brainstorms cast over the caster's own spell). Check the stack's controller.
- **The view's stack carries its targets now** (S32). Anything that asks "is this creature targeted" reads them; before S32 that field only existed in tests (`as never`), so `creatureIsDoomed` was blind live for three sessions. When a test builds a view field the real `buildView` does not produce, the agent code reading it is untested.
- **Measure before you tune, even a "correction".** The S32 Buried Alive and Zombify fixes each came from one 200-game scratch script counting the actual picks; the sweep's cast counts would never have shown either (a reanimated card is not a cast — hence `facts.returned`).
- **Modifiers apply after mulligans** (ADR-002): a test that wants the opening board with `permanentOnBattlefield` basics must capture at the first PRIORITY request, not the first request.
- **The matrix pattern for a worldcraft lever**: parameterise the sweep's Side (life + `entrance`), keep the catalog untouched, and let `pairing()` return its numbers so the aggregate table is computed in the same run (S33 part 8). A knob argument is only as good as the table it is read from.
