...
- **Duel logs are downloadable now**: the viewer's transport bar has "Download log" (`duel-<seed>-<turns>t.json`), and "⟵ Back" when the viewer was launched from a game. The journey stays MOUNTED (hidden) under the replay — Back returns to the exact screen that launched it (the result panel included); `WorldApp`'s `paused` prop holds the screen-driven music effect still meanwhile. The world's "Recent duels" rail panel keeps the last six duels' logs, so a report can be filed after the fact.
