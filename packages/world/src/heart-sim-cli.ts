...
const references: Ref[] = refFilter === "flood" ? floodRefs : refFilter === "postlairs" ? [ROAD_DECKS.salvageWRLordsLairs!, ROAD_DECKS.salvageUBLordsLairs!].map((r) => ({ name: r.name, archetype: r.archetype, decklist: r.decklist, profile: "journeyman" as const, life: r.life, entrance: [...r.entrance] })) : refFilter === "postlords" ? [ROAD_DECKS.salvageWRLords!, ROAD_DECKS.salvageUBLords!].map((r) => ({ name: r.name, archetype: r.archetype, decklist: r.decklist, profile: "journeyman" as const, life: r.life, entrance: [...r.entrance] })) : refFilter === "stock" ? stock : refFilter === "road" ? [road] : [...stock, road];
