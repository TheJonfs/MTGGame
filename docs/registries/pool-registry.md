...
| angel_of_the_ruins | Angel of the Ruins | tested | artifact creature, flying; triggered(self ETB) targets {0..2} artifactOrEnchantment: exile targetSpec 0; cycling {2} + cyclingSearch subtype:Plains (plainscycling) | BRC printing (The Brothers' War Commander, 2022 — Chris's pick for the old frame; the earliest is C21 2021); tier 3 / 50g (priceOverride). Plainscycling fetches a Godless Shrine (revealed); the ETB exiles a law token and an opposing Control Magic (our creature comes home); zero targets resolves; Disenchant destroys it (fixtures). AI (book 53): cycle with a reanimator in hand or short on lands by turn three; the ETB takes their aura on our creature first, never our own. Corvane ×1 (over the third Serra). |
