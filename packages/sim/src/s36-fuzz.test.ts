...
/**
 * S36 fuzz-before-fixtures: Thawing Glaciers (the cleanup return, the tapped fetch), Library of Alexandria
 * (the hand-size activation condition), Arcane Collector (name-and-reveal), Angel of the Ruins (plainscycling,
 * the up-to-two exile) and the five amended lists (ADR-121) under random play — every mage against a mage and
 * a starter, both seats, the starters' round-robin; replays byte-identical. The Library is shop-only: it rides
 * a test deck here (Tessaly with two Libraries) so the condition is exercised.
 */
describe("S36 fuzz — the Glaciers, the Library, the Collector, the Angel; the five lists (fuzz-before-fixtures)", () => {
  ...
  it(`${games} games × 15 mages × 2 references × 2 seats + the starters' round-robin: zero exceptions, every game terminates`, async () => {
    ...
    for (const [k, mage] of Object.entries(MAGE_DECKS)) {
      ...
      const other = MAGE_DECKS[keys[(keys.indexOf(k) + 13) % keys.length]!]!;
      ...
      for (const [refName, ref] of [[other.name, other.decklist], [`starter:${starter.id}`, starter.decklist]] as const) {
        for (let seed = 1; seed <= games; seed++) {
          for (const seat of [0, 1] as const) {
            ...
            const result = await runMatch({ seed: seed * 37 + seat + 3600, players, rules: RULES, modifiers: [] }, cards, [new RandomAgent(seed * 2 + 1), new RandomAgent(seed * 2 + 2)]);
    ...
    for (let i = 0; i < starters.length; i++) {
      for (let j = i + 1; j < starters.length; j++) {
        for (let seed = 1; seed <= games; seed++) {
          ...
          const result = await runMatch({ seed: seed * 41 + i * 5 + j + 3600, players: [{ name: a.name, decklist: a.decklist, agent: "random" }, { name: b.name, decklist: b.decklist, agent: "random" }], rules: RULES, modifiers: [] }, cards, [new RandomAgent(seed * 2 + 1), new RandomAgent(seed * 2 + 2)]);
  ...
  it("replay determinism — the decks with the new cards (Tessaly, Kessa, Corvane, Pell, Quill) replay byte-identical", async () => {
    const cards = loadCardPool(CARDS_DIR).cards;
    for (const [k, seed] of [["tessaly", 51], ["kessa", 52], ["corvane", 53], ["pell", 54], ["quill", 55]] as const) {
