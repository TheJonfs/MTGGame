...
*Pushed to origin/main on 2026-09-26 (the Altar / lair-dungeons / clock slack round, the art round with its two corrections); `pnpm build:web` run clean first — a push deploys to Vercel.*
...
## Post-session (Chris's first phase-two play, 2026-09-26): three more

1. **The Altar of Dementia and the creature about to die — confirmed and fixed.** The gate's "never our last untapped blocker while behind" clause counted the doomed creature as the blocker it was saving, so a lone (or last untapped) creature under removal was never cashed. Now a doomed creature is fed whatever the board; a sweeper on the stack (Pyroclasm's damage reaching its toughness, Wrath) dooms too; of several doomed the biggest body is fed (book 37 extended: the lone Terror target, the Pyroclasm, the Wrath, and never our own spell). Ladder PASS, both gates.
2. **The flood's lairs are LAIR-DUNGEONS** (the S14 single duel lived one session). Stepping onto one opens the dungeon telegraph (the lair's line and what it holds, the resident named — "the surprise until the telegraph" kept); the crawl, its caches (gold, cards, interior life, boons), the escrow and the interior clock are phase one's; the resident guards the deep end at its **phase-two row + the lair bonus with its entrance basics** (`lairGuardian`); the fall pays **the prize room (30 gold + two R cards, `lairPrizeRoll`) AND the manalink** — the ceremony's notes carry the link's line. The run's colour is the territory's. Retrieval quests post on the flood again (the prize room exists). Fuzz: the fifteen guardian duels on two maps with the post-lords pilots, replays byte-exact (1 win in 30 at the salvage world's life — the row is the same as before); the controller test walks a lair to its deep end. **Watch item for the planner**: fifteen lairs × two R cards is thirty R cards a map (phase one's five lairs paid ten) beside the golds; `lairPrizeRoll` is shared — a flood-specific roll is a small change if that is rich.
3. **The clocks, mildly** — `floodClockSlack` **1.5** (⚠ unratified): in a phase-two world the first siege threat schedules from 450 steps instead of 300 (the interval and the cadence after it unchanged), and a lord grows a life every 150 steps instead of 100 (the cap unchanged). Pinned (the same seeded town 150 steps later; 2 life at 300 steps, not 3).

## Post-session (Chris's art round, 2026-09-26): twelve real cards in our art, seventeen faces

Chris delivered card art (his Gemini pipeline) for **Restoration Angel, Drana, Kalastria Bloodchief and the ten ABU duals**, plus as-printed faces for those twelve and for **the five laws**. Installed: the art as 5:4 crops (the house 35% bias) at 1024×819 in `assets/generated/card-art/<id>.png` + `packages/ui/public/custom-art/` (Chris's full-size sources stay beside them as `<id>.jpeg`); the faces at 745px in `assets/generated/printed/` + `packages/ui/public/printed-art/`; the defs carry `art.asset` (the twelve) and `printedAsset` (sixteen). **Two code changes**: (1) **ADR-082 amended** — a REAL card may carry a `printedAsset` (its face replaces the Scryfall scan on printed surfaces) and a real card with `art.asset` wears OUR art in the frame, the hand frame and the tiles (`CardFrame`, `CardTile`, `Gallery`'s strip; the Scryfall derivatives stay for every other real card); (2) the printed toggle (the inspector, the rail, the gallery modal) now offers a custom face too — the laws' and the legends' faces were reachable only through the gallery's "printed scans" before. Verified word-for-word against the oracle/defs: **sixteen clean**; **Drana's face is HELD** — it prints "Target creature**s** gets −0/−X" (the oracle: "Target creature gets"); her art is installed, the Scryfall scan stays her printed view until a corrected render (ledgered HELD). The Toll's face was re-delivered the same day with the def's wording ("to that player") and re-installed. Drana's face took three renders ("creatures", then −X/−0 for −0/−X); the third is word-for-word and installed — all seventeen faces are in. Walked in the gallery: Tundra in our frame with the new art and as its face; the Toll's face and its toggle.
