...
describe("card pool loading", () => {
  it("loads the full pool (S1–S5 additions + tokens) with no errors or warnings", () => {
    ...
    expect(pool.cards.size).toBe(242) // S42a: +2 (ADR-131/133: the Cinquefont — custom, prizeOnly; Time Walk — LEA, prizeOnly, the flood's only Power); // S40: +27 (ADR-128, the flood's cards: ten legends + five High Grounds prizeOnly; Static Sphere, Sacred Helix and eight real golds at R; Char and Shadow Summoning T2) + 2 tokens (Snake, Spirit); // S36: +4 (ADR-121: Thawing Glaciers T2, Library of Alexandria R, Arcane Collector T2, Angel of the Ruins T3); // S28: +5 one-drops (ADR-098); S29: +6 (ADR-101) + the Elemental token; S30: +6 (ADR-104); S31: +2 (ADR-108: Artisan of Kozilek R, Grazing Gladehart T1); S32: +2 (ADR-109: Plumecreed Escort T1, Diabolic Edict T2); // 20 S1 + 11 S2 + 11 S3 + 16 S4 + 6 S5 + 2 tokens + 1 S8 (Cunning Tactician) + 3 S15 (Growth, Tutor, Lotus) + 2 S16 (Elves, Adept) + 32 S17 (Expansion 1) + 5 tokens (Bear, Bird, Wurm, Zombie, Faerie Rogue) + 2 S19 (Faerie Formation + blue Faerie token, ADR-078; manalinks are plain basics via permanentOnBattlefield — round 2 cut the bespoke defs) + 36 S20 (20 duals — ABU at R/OLGC, RVR retro shocks at T2/45g with entersChoice; 6 enabler lands; Reya/Arcanis/Drakuseth/Titania — prizeOnly since ADR-081, as is Drana; 5 Moxen prizeOnly; elemental token) + 19 S22a (A10 batch: five lords prizeOnly + Aetherbolt + Tainted Phoenix at R; eight real gold→R adds; Abrade T1/12g; saproling/sphinx/weird tokens) + 5 S22b (the stronghold laws — uncastable prizeOnly Artifact Enchantments in data/cards/laws/) + 3 S23 (the fun batch: Thundersnake, Gallows Djinn, Traumatizer — all custom, tier 2) + 5 S25 (the Mox court — Pearl Cleric, Sapphire Sage, Jet Witch, Ruby Tyrant, Emerald Keeper; all custom prizeOnly, ADR-088)
...
describe("validateCard rejections", () => {
  ...
  it("every vocabulary word has a resolver as of S17 (untapTarget was the last — Little Bear); the no-resolver warning path stays for future words", () => {
    const staticOnly = new Set(["grantKeyword", "gainControl", "grantAbility", "extraLandDrops", "imposeEntersTapped"]); // interpreted live by characteristics()/abilitiesOf()/maxLandDrops()/imposedEntersTapped(); no resolver by design
